...
export class TestSetup {
    ...
    try {
      ...
      if (process.env.AUTH_USERNAME && process.env.AUTH_PASSWORD) {
        ...
        const authResponse = await restified
          ...
          .when()
            .post('/auth/login') // or /oauth/token, /api/v1/auth, etc.
            .execute()
          .then()
            .statusCode(200)
            .extract('$.access_token', 'authToken') // Extract token and store as variable
            .extract('$.refresh_token', 'refreshToken') // Also extract refresh token if available
            .extract('$.expires_in', 'tokenExpiresIn')
            .execute();
      ...
      // Option 3: OAuth2 Client Credentials flow (for service-to-service)
      if (process.env.OAUTH2_CLIENT_ID && process.env.OAUTH2_CLIENT_SECRET) {
        ...
        const oauth2Response = await restified
          .given()
          ...
          .then()
            ...
            .extract('$.token_type', 'tokenType') // Usually 'Bearer'
            .extract('$.expires_in', 'tokenExpiresIn')
            .execute();
      ...
      // Option 4: API Key authentication (simpler pattern)
  ...
  /**
   * Refresh token if needed (called before tests that require fresh tokens)
   */
  static async refreshTokenIfNeeded() {
    const tokenExpiresIn = restified.getVariable('tokenExpiresIn');
    const refreshToken = restified.getVariable('refreshToken');
    
    if (tokenExpiresIn && refreshToken && this.isTokenExpiringSoon(tokenExpiresIn)) {
  ...
  private static isTokenExpiringSoon(expiresIn: number): boolean {
