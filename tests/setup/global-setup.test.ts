import { executeGlobalCleanup, forceCleanup } from '../helpers/cleanup';
...
describe('Global Setup and Teardown Tests @integration @setup', () => {
  ...
  after(async function () {
    ...
    try {
      ...
      // 4. Execute comprehensive cleanup to prevent process hanging
      console.log('🔧 Step 4: Executing comprehensive cleanup...');
      try {
        await executeGlobalCleanup();
        console.log('✅ Global cleanup tasks completed');
      } catch (cleanupError: any) {
        console.warn('⚠️  Global cleanup warning:', cleanupError.message);
      }

      // 5. Force cleanup of any remaining resources
      console.log('🚨 Step 5: Force cleanup of remaining resources...');
      try {
        forceCleanup();
        console.log('✅ Force cleanup completed');
      } catch (forceError: any) {
        console.warn('⚠️  Force cleanup warning:', forceError.message);
      }
      ...
      // Even if teardown fails, attempt cleanup
      try {
        await executeGlobalCleanup();
        forceCleanup();
      } catch (finalCleanupError) {
        console.error('❌ Final cleanup attempt failed');
      }
    } finally {
      // --- FINAL DIAGNOSTIC AND FORCED EXIT ---
